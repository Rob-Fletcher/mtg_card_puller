...
def extract_card_id(mox_full_name: str) -> Tuple[str, str, int]:
    ...
    card_series = ""
    card_number = ""
    match = re.search(r"\d (.*) \(([A-Z0-9]{3,4})\) ([A-Z0-9\-]*)", mox_full_name)
    if match:
        ...
        logger.debug(
            f"Extracted card series: {card_series}, card number: {card_number} from full name: {mox_full_name}"
        )
    else:
...
def execute_request(card_name: str, card_series: str, card_number: int) -> None:
    ...
    """
    normalized_name = card_name.strip(".'").replace(" ", "_").replace("/", "")
    ...
    r = requests.get(
        f"https://api.scryfall.com/cards/{card_series}/{card_number}", headers=headers
    )
    if r.status_code != 200:
        logger.error(
            f"Failed to fetch card details for {card_name} ({card_series} {card_number})"
        )
        return

    img_url = r.json().get("image_uris", {}).get("png")
    if not img_url:
        card_faces = r.json().get("card_faces", [])
        if card_faces == []:
            logger.error(
                f"No image found for {card_name} ({card_series} {card_number})"
            )
            return
        img_url = [face.get("image_uris", {}).get("png") for face in card_faces]
    ...
    for i, url in enumerate(img_url):
        ...
        if r.status_code != 200:
            logger.error(
                f"Failed to fetch image for {card_name} ({card_series} {card_number})"
            )
            return

        # Save the image to a file
        save_name = (
            f"{normalized_name}_{i}" if len(img_url) > 1 else f"{normalized_name}"
        )
...
def main(deck_file: str, verbose: bool = False) -> None:
    ...
    with open(deck_file, "r") as f:
        pbar = tqdm.tqdm(
            f.readlines(), desc="Processing cards", colour="green", unit="card"
        )
        for line in pbar:
            try:
